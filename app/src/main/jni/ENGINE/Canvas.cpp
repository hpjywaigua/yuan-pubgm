...
//Canvas::Canvas(JNIEnv *env, int width, int height) {
    this->env = env;
...
void Canvas::drawAlertbox(float startx,float startb,float enda,float endb,int color){
    Paint *paint = this->m_FillPaint;

    paint->setColor(color);

    env->CallVoidMethod(this->m_CanvasObj, this->drawRectId, startx, startb, startx + (enda-startx), startb + (endb-startb), paint->paintObj);

}
void Canvas::drawLines(Vector2 X, Vector2 Y, float thicc, int color) {
...
void Canvas::drawAlertboxBorder(float startx,float startb,float enda,float endb,float thicc,int color){
...
void Canvas::drawBorder(Vector2 Pos, float width, float height, int color) {
    Paint *paint = this->m_FillPaint;

    paint->setColor(color);

    env->CallVoidMethod(this->m_CanvasObj, this->drawRectId, Pos.X, Pos.Y, Pos.X + width, Pos.Y + height, paint->paintObj);
}
...
void Canvas::drawLine2(Vector2 start, Vector2 end,float thicc, int color) {
    Paint *paint = this->m_LinePaint;
...
void Canvas::draw3D(int color, float thickness, Vector2 start, Vector2 stop){
    Paint *paint = this->m_LinePaint;
...
void Canvas::drawBox4Line(float thicc, int x, int y, int w, int h, int color) {
    ...
    int ih = h / 4;
    // top
    drawRect(Vector2(x, y),Vector2(x + iw, y), thicc, color);
    drawRect(Vector2(x + w - iw, y),Vector2(x + w, y), thicc, color);
    drawRect(Vector2(x, y),Vector2(x, y + ih), thicc, color);
    drawRect(Vector2(x + w - 1, y),Vector2(x + w - 1, y + ih), thicc, color);
    // bottom
    drawRect(Vector2(x, y + h),Vector2(x + iw, y + h), thicc, color);
    drawRect(Vector2(x + w - iw, y + h),Vector2(x + w, y + h), thicc, color);
    drawRect(Vector2(x, y + h - ih), Vector2(x, y + h), thicc, color);
    drawRect(Vector2(x + w - 1, y + h - ih), Vector2(x + w - 1, y + h), thicc, color);
}

void Canvas::DrawBox(float thicc, Rect2 rect,int color) {
        ...
        drawLine2(v1, v2,thicc, color);
        drawLine2(v2, v3,thicc, color);
        drawLine2(v3, v4,thicc, color);
        drawLine2(v4, v1,thicc, color);
    }
