...
//Canvas(JNIEnv *env, int width, int height);
	...
	void DrawBox(float startX, Rect2 rect, int color = 0xFFFFFFFF);
	void draw3D(int color, float thickness, Vector2 start, Vector2 stop);
    ...
    void drawLine2(Vector2 start, Vector2 end,float thicc, int color = 0xFFFFFFFF);
	...
	void drawBorder(Vector2 Pos, float width, float height, int color);
void drawNew3DBox(SDK::FVector extends, SDK::FVector origin,float thickness,int color);///Add this in 'Canvas.h' too.
void drawAlertbox(float startx,float startb,float enda,float endb,int color);
    void drawAlertboxBorder(float startx,float startb,float enda,float endb,float thicc,int color);
	    void drawLines(Vector2 X, Vector2 Y, float thicc, int color);
	//RectFilled
