...
extern "C"
{
	...
	/*查找文本 参数：被查找文本 查找文本 结束文本 是否使用静态空间*/
	char *strstrstr(const char *str, const char *front, const char *rear, bool isStatic); //变态级的函数,看了可能会崩溃
	int 请求(const char *hostname, const char *url, const char *cs, char **run);	  //POST方法
	int httpget(const char *hostname, const char *url, char **run);						  //GET方法
	char *getip(const char *hostname);													  //域名转换IP
...
int 请求(const char *hostname, const char *url, const char *cs, char **run)
{
	// sock句柄
	int sockfd;
...
//		perror("setsockopt sockfd");
	...
	{
		printf("ERROR:连接到服务器失败,connect error!\n");
		close(sockfd);
		return 0;
	}
	//因为懒，所以char[2048]
...
*/
	...
	{
		printf("ERROR:发送失败！错误代码:%d，错误信息:%s\n", errno, strerror(errno));
		close(sockfd);
		return 0;
	}
...
int code = select(sockfd + 1, &fds, NULL, NULL, &tv);
	if (code < 1)
	{
		//非阻塞轮询时使用continue;
		//-1错误
	...
	{ //测试sock是否可读，即是否网络上有数据
		...
		if (!readlen)
		{
			printf("ERROR:服务器断开了连接，并未发送任何数据。\n");
			close(sockfd);
			return 0;
		}
//		printf("\n-- 回复协议头 --\n%s\n-- END --\n",xyt);
		...
		if (xylen == NULL)
		{
			...
			*hex = '\0';
			...
			//rdlen++;
			xyzw = (char *)malloc(readlen + 2); //把\r\n读出来
			read(sockfd, xyzw, readlen + 2);	//方便下面读取区块信息
			//xyzw[rdlen] = '\0';
			//读取下一块内容
			while (1)
			{
				...
				*hex = '\0';
				...
				if (chlen == 0)
					break;
				readlen += chlen;
				xyzw = (char *)realloc(xyzw, readlen + 2);
				char *xrzz = xyzw + readlen - chlen;
...
int httpget(const char *hostname, const char *url, char **run)
{
	// sock句柄
	int sockfd;
	struct sockaddr_in serveraddr;
